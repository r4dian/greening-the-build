...
namespace greeningthebuild
{
    class MainClass
    {
        ...
        private static APIClient ConnectToTestrail()
        {
            ...
            client.User = "___ EMAIL @ DOMAIN.TLD ___";
            client.Password = "___ Obtain from testrail or ask @andrewse ___ "; //API key
            return client;
        }
        ...
        public static List<Test> CreateListOfTests(APIClient client, JArray testsArray, string projectID)
        {
            ...
            for (int i = 0; i < testsArray.Count; i++)
            {
                ...
                // Skip couple of tests which are reported with wrong Editor Version, which can't be corrected as well since the Runs are closed now.
                if (testID.Equals("1028826") || testID.Equals("1028827") || testID.Equals("1028925"))
                {
                    Console.WriteLine("Skipping Tests which has wrong Editor Version results.." + testID);
                }
                else
                {
                    Test test;
